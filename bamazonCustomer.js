    ...
    inquirer.prompt([
		...
		connection.query('SELECT * FROM products WHERE item_id=?', purchased[0].ItemID, function(err, res){
				...
				if(res[0].stock_quantity < purchased[0].Quantity){
					console.log(colors.red('Beeb boop beep beep...Sorry that item is out of stock! #sopoplular'));
					connection.end();

				//otherwise if the stock amount available is more than or equal to the amount being asked for then we are a-go
					...
					//this creates the variable SaleTotal 
					...
					connection.query("UPDATE Products SET StockQuantity = " + newQuantity +" WHERE ItemID = " + purchased[0].itemID, function(err, res){
						...
						console.log('');
						console.log(colors.cyan('Yay! Your order is on the way to be shipped to you! Thanks for shopping with BAmazon! Now go wait by your door!'));
						console.log('');
